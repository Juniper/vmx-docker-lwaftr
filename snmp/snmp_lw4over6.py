...
def populate_stats():
    try:
        ...
        if (refresh_file == 1): 
            ...
            #state_fp.write(etree.tostring(root))
        ...
        jcs.syslog("external.info", "Before looping")
        root = state_rsp
                ...
                for child in instance:
                    if (child.tag == 'lwaftr'):
                        ...
                        for elem in child:
                            ...
                            if (value):
                                lwaftr_entry.lwaftrV4HCInOctets = value
                                continue
                            value = populate_node_value(elem, 'in-ipv4-packets')
                            if (value):
                                lwaftr_entry.lwaftrV4HCInPkts = value
                                continue
                            value = populate_node_value(elem, 'out-ipv4-bytes')
                            if (value):
                                lwaftr_entry.lwaftrV4HCOutOctets = value
                                continue
