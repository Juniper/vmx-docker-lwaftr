...
docker exec -ti $CONTAINER bash -c "cd /tmp; (ps axw && lscpu && cat /proc/meminfo && /show_affinity.sh) > sysinfo.txt; tar zcf /tmp/support-info.tgz /VERSION *.log mac_* pci_* *txt *txt.s *.cfg *.conf stats.xml ../config_drive/vmm-config.tgz ../root/.bashrc" 
