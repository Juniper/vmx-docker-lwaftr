...
sudo snabb lwaftr run --conf snabbvmx-lwaftr-${INT}.conf --on-a-stick $PCI
EOF
...
sudo nabb snabbvmx lwaftr --conf test-snabbvmx-lwaftr-${INT}.cfg --id $INT --pci $PCI --mac $MAC
EOF
  chmod a+rx test_snabb_*sh

  echo "launch snabbvmx for $INT on cpu $CORE (node $NODE) after $SLEEP seconds ..."
  CMD="$NUMACTL snabb snabbvmx lwaftr --conf snabbvmx-lwaftr-${INT}.cfg --id $INT --pci $PCI --mac $MAC --sock ext$INT --mirror tap${INT:2:1}"
  echo $CMD
  sleep $SLEEP
