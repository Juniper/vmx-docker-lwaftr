...
EOF
  ...
  if [ -z "`grep lwaftrgen snabbvmx-lwaftr-${INT}.cfg`" ]; then
    CMD="$NUMACTL $SNABB snabbvmx lwaftr --conf snabbvmx-lwaftr-${INT}.cfg --id $INT --pci $PCI --mac $MAC --sock %s.socket --mirror $INT"
  else 
    CMD="$NUMACTL $SNABB snabbvmx lwaftrgen --conf snabbvmx-lwaftr-${INT}.cfg --id $INT --pci $PCI --mac $MAC --sock %s.socket"
  fi
  echo $CMD
  sleep $SLEEP
